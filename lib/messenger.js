...
class Messenger {
    ...
    async handleText (target, context, msg, self) {
        ...
        if (textSplit.length > 0 && inputParser.isAskingForTFMap(textSplit[0])) {
            return handlers.tempsDeFlors.getMapLink(target, this.bot)
        }

        if (textSplit.length > 0 && inputParser.isAskingForTFHelp(textSplit[0])) {
            return handlers.tempsDeFlors.getHelpLink(target, this.bot)
        }
        ...
        if (textSplit.length === 1 && inputParser.isAskingForTFActiveSpot(textSplit[0])) {
            return handlers.tempsDeFlors.getActive(target, this.bot, context['room-id'])
        }
