...
class Messenger {
    ...
    async tokenAutoRefresh () {
        ...
        this.authProvider.onRefresh(( newTokenData) => {
            TokenService.updateToken(config.twitch.userId, newTokenData)
        })
        await this.authProvider.addUserForToken(tokenData)
        this.authProvider.addUser(parseInt(config.twitch.userId), tokenData)
    }
