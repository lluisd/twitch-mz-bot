...
class Messenger {
    ...
    async tokenAutoRefresh () {
        logger.info('tokenAutoRefresh with ' + config.twitch.clientId + ' - ' + config.twitch.clientSecret)
        ...
        logger.info('Twitch token initialized, tokendata:' + tokenData + ' for userId ' + config.twitch.userId)
