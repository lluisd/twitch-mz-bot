...
class Messenger {
    ...
    async handleText (target, context, msg, self) {
        ...
        if (textSplit.length > 1 && inputParser.isAskingToTimeoutUser(textSplit[0]) && this._isAdmin(context)){
            return handlers.ban.ban(target, textSplit[1], this.bot, textSplit[2] || 600)
        }

        if (textSplit.length > 1 && inputParser.isAskingToBanUser(textSplit[0]) && this._isAdmin(context)){
            return handlers.ban.ban(target, textSplit[1], this.bot)
        }

        if (textSplit.length > 1 && inputParser.isAskingTOUnbanUser(textSplit[0]) && this._isAdmin(context)){
            return handlers.ban.unban(target, textSplit[1], this.bot)
        }

        if (textSplit.length > 1 && inputParser.isAskingToSetTitle(textSplit[0]) && this._isEditor(context)){
            return handlers.generic.setTitle(target, textSplit.slice(1).join(' '), this.bot)
        }

        if (textSplit.length > 1 && inputParser.isAskingToSetGame(textSplit[0]) && this._isEditor(context)){
