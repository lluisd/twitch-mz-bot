...
class Messenger {
    ...
    async handleText (target, context, msg, self) {
        ...
        if (textSplit.length > 0 && inputParser.isAskingForRollDice(textSplit[0]))
            return handlers.generic.rollDice(target, this.twitchBot)

        if (textSplit.length > 0 && inputParser.isAskingForServerStatus(textSplit[0]))
            return handlers.generic.status(target, this.twitchBot)

        if (textSplit.length > 0 && inputParser.isAskingForOF(textSplit[0]) && this._isNotCooldown('of',15))
            return handlers.generic.randomYoutubeLink(target, this.twitchBot)
        ...
        if (textSplit.length > 1 && inputParser.isAskingForSunset(textSplit[0]))
            return handlers.weather.getSunset(target, textSplit.slice(1).join(' '), this.twitchBot)

        if (textSplit.length > 1 && inputParser.isAskingForSunrise(textSplit[0]))
            return handlers.weather.getSunrise(target, textSplit.slice(1).join(' '), this.twitchBot)


        // Birthday
        if (textSplit.length === 2 && inputParser.isAskingForAddBirthday(textSplit[0]) && textSplit[1].includes('-'))
            return handlers.birthday.addBirthday(target, textSplit.slice(1).join(' '), this.twitchBot, context['display-name'])

        if (textSplit.length === 2 && inputParser.isAskingForGetBirthday(textSplit[0]) && !textSplit[1].includes('-'))
            return handlers.birthday.getBirthday(target, textSplit[1], this.twitchBot)


        // Temps de Flors
        if (config.features.TFSpots && textSplit.length > 1 && inputParser.isAskingForTFSpot(textSplit[0])
            && this._isNotCooldown('tf', 15))
            return handlers.tempsDeFlors.getSpot(target, textSplit[1], this.twitchBot, context['room-id'])

        if (config.features.TFSpots && textSplit.length > 0 && inputParser.isAskingForTFSpotsCount(textSplit[0])
            && this._isNotCooldown('tf', 15))
            return handlers.tempsDeFlors.getTotalSpot(target, this.twitchBot, context['room-id'])
        ...
        if (config.features.TFSpots && textSplit.length > 1 && inputParser.isAskingForTFVisited(textSplit[0]) &&
            ...
            return handlers.tempsDeFlors.setVisited(target, textSplit[1], this.twitchBot, context['room-id'], true)
        ...
        if (config.features.TFSpots && textSplit.length > 0 && inputParser.isAskingForTFScreenshot(textSplit[0]) &&
            ...
            return handlers.tempsDeFlors.captureScreenshot(target, textSplit[1], this.twitchBot, context['display-name'], context['room-id'])

        // Stream
        if (textSplit.length > 0 && inputParser.isAskingForTakeScreenshot(textSplit[0]) && this._isNotCooldown('screenshot', 15))
            return handlers.stream.captureScreenshot(target, this.twitchBot, context['display-name'], context['room-id'])

        if (textSplit.length > 0 && inputParser.isAskingForF5(textSplit[0]) &&
            (isVip(context) ||isMod(context) || isBroadcaster(context)))
            return handlers.stream.refreshPage()
        ...
        if (textSplit.length > 0 && inputParser.isAskingBans(textSplit[0]) &&
            ...
            return handlers.ban.getBansCountAndUnbanRequests(target, this.twitchBot)

        if (textSplit.length > 0 && inputParser.isAskingLastTimeouts(textSplit[0]) && (isAdmin(context) || isMod(context) || isBroadcaster(context)))
            return handlers.ban.getTimeouts(target, this.twitchBot)

        if (textSplit.length > 0 && inputParser.isAskingUnbanRoulette(textSplit[0]) && (isVip(context) || isAdmin(context) || isMod(context) || isBroadcaster(context)) && this._isNotCooldown('unbanRoulette',15))
            return handlers.ban.unbanRoulette(target, this.twitchBot)
        ...
        if (textSplit.length > 1 && inputParser.isAskingOpenAI(textSplit[0]) && this._isNotCooldown('openai',15)) {
            return handlers.openAI.askOpenAI(target,  textSplit.slice(1).join(' '), context['display-name'], this.twitchBot)
        }
        ...
        if (textSplit.length > 1 && inputParser.isAskingBotOpenAI(text) && this._isNotCooldown('openai',15)) {
            ...
            return handlers.openAI.askOpenAI(target, textWithoutMention, context['display-name'], this.twitchBot)
        }
