...
class Messenger {
    ...
    async handleText (target, context, msg, self) {
        ...
        if (textSplit.length > 1 && inputParser.isAskingToTimeoutUser(textSplit[0])){
            return handlers.ban.ban(target, textSplit[1], this.bot, textSplit[2] || 600)
        }

        if (textSplit.length > 1 && inputParser.isAskingToBanUser(textSplit[0])){
            return handlers.ban.ban(target, textSplit[1], this.bot)
        }

        if (textSplit.length > 1 && inputParser.isAskingTOUnbanUser(textSplit[0])){
            return handlers.ban.unban(target, this.bot)
        }

        if (textSplit.length > 1 && inputParser.isAskingToSetTitle(textSplit[0])){
