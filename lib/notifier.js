...
class Notifier {
    ...
    notify () {
        ...
        cron.schedule('* * * * *', async () => {
            try {
                await handlers.ban.checkExpiredImmunes(this.target, this.twitchBot)
            } catch (err) {
                logger.error(err)
            }
        })
        ...
        cron.schedule('36 4 * * * *', async () => {
            try {
                await TwitchService.updateVips()
                logger.info('Vips updated')
            } catch (err) {
                logger.error(err)
            }
        })
        ...
        cron.schedule('6 6 * * * *', async () => {
            try {
                await TwitchService.updateMods()
                logger.info('Mods updated')
            } catch (err) {
                logger.error(err)
            }
        })
        ...
        cron.schedule('40 * * * *', async () => {
            try {
                ...
                if (channel.live) {
                    const text = 'como si fueras un usuario del chat inventate alguna curiosidad reciente del streamer o de su chat anecdótica o de dato curioso para compartir con el chat'
