...
class EventSub {
    ...
    async immuneSlotRedemptionHandler (slotNumber, event) {
        try {
            ...
            if (channel && channel.whitelistUsers.includes(parseInt(event.userId))) {
                ...
                await TwitchService.cancelRedemption(event.rewardId, event.id, event.status)
            } else if (immune) {
                logger.info(`User ${event.userDisplayName} is whitelisted, ignoring redemption...`);
                await this.twitchBot.say(`#${config.twitch.channels}`, `@${event.userDisplayName} ya eres un usuario de confianza, no puedes canjear el inmune!`)
                await TwitchService.cancelRedemption(event.rewardId, event.id, event.status)
                ...
                await TwitchService.acceptRedemption(event.rewardId, event.id, event.status)
            }
        } catch (err) {
    ...
    async subscribeEvent (channelId) {
        ...
        immuneSlotsRewards.forEach((reward, index) => {
            ...
            if (!rewardExists) {
                console.warn(`El rewardId immuneSlot${index + 1} ${reward.id} no existe, se omite la suscripción.`);
                return
            }
        ...
        if (!setTitleRewardExists) {
            console.warn('El rewardId setTitle ' + config.customReward.setTitle + ' no existe. La suscripción no se creará.')
        } else {
        ...
        if (!buyVipRewardExists) {
            console.warn('El rewardId buyVip ' + config.customReward.buyVip + ' no existe. La suscripción no se creará.')
        } else {
        ...
        if (!stealVipRewardExists) {
            console.warn('El rewardId stealVip ' + config.customReward.stealVip + ' no existe. La suscripción no se creará.')
        } else {
        ...
        if (!unbanRewardExists) {
            console.warn('El rewardId unban ' + config.customReward.unban + ' no existe. La suscripción no se creará.')
        } else {
                ...
                try {
                    ...
                    if (event.input && event.input.length > 1) {
                        ...
                        if (!isBanned && !isBlocked) {
                            logger.info(`${targetUser.display_name} is not banned or blocked`)
                            await TwitchService.cancelRedemption(event.rewardId, event.id, event.status)
                            await this.twitchBot.say(`#${config.twitch.channels}`, `@${event.userDisplayName} unban fallido, @${targetUser.display_name} no está ban!`)
                            return
                        }
                        ...
                        logger.info(`user ${event.userDisplayName} unbanned and unblocked ${targetUser.display_name}`)
                        await this.twitchBot.say(`#${config.twitch.channels}`, `@${event.userDisplayName} ha desbaneado a @${targetUser.display_name}`)
                        await TwitchService.acceptRedemption(event.rewardId, event.id, event.status)
                    } else {
                        logger.info(`${event.input} no es valido para canjeo`)
                        await TwitchService.cancelRedemption(event.rewardId, event.id, event.status)
        ...
        if (!timeoutRewardExists) {
            console.warn('El rewardId timeout ' + config.customReward.timeout + ' no existe. La suscripción no se creará.')
        } else {
