...
class EventSub {
    ...
    async subscribeEvent (channelId) {
        ...
        this.middleware.onChannelSubscription(channelId, async event => {
            try {
                logger.info(`${event.userDisplayName} just subscribed to ${event.broadcasterDisplayName}! (isGift: ${event.isGift})`);
                if (!event.isGift) {
                    await this.twitchBot.say(`#${config.twitch.channels}`, `Tenemos nueva sub ${event.isGift ? '(regalada) ' : ''}@${event.userDisplayName} masónica!`)
                }
            } catch (error) {
