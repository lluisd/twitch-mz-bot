                ...
                try {
                    ...
                    if (event.input && event.input.length > 1) {
                        ...
                        if (!targetUserIsVip) {
                            ...
                            await event.updateStatus('CANCELED')
                            await this.twitchBot.say(`#${config.twitch.channels}`, `@${event.userDisplayName} revocado fallido, @${targetUser.display_name} no es VIP!`)
                            return
                        }
                        await TwitchService.removeVip(targetUser.id)
                        logger.info(`Revoked VIP from ${targetUser.display_name}`)
                        await this.twitchBot.say(`#${config.twitch.channels}`, `@${event.userDisplayName} ha revocado el VIP de @${targetUser.display_name}`)
                        await event.updateStatus('FULFILLED')
                    } else {
                ...
                try {
                    ...
                    if (event.input && event.input.length > 1) {
                        ...
                        if (!targetUserIsVip) {
                            logger.info(`${targetUser.display_name} is not a VIP`)
                            await event.updateStatus('CANCELED')
                            await this.twitchBot.say(`#${config.twitch.channels}`, `@${event.userDisplayName} robo fallido, @${targetUser.display_name} no es VIP!`)
                            return
                        }
                        await TwitchService.removeVip(targetUser.id)
                        await TwitchService.addVip(event.userId)
