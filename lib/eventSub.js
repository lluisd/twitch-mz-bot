...
class EventSub {
    ...
    async subscribeEvent (channelId) {
        ...
        this.middleware.onChannelRedemptionAddForReward(channelId, config.customReward.buyMod, async event => {
            logger.info(`${event.userDisplayName} ha canjeado ${event.rewardTitle}!`)
            const isAlreadyMod = await TwitchService.isMod(event.userId)
            if (!isAlreadyMod) {
                await TwitchService.addMod(event.userId)
                logger.info(`Added Mod to ${event.userDisplayName}`)
                await this.twitchBot.say(`#${config.twitch.channels}`, `@${event.userDisplayName} ha comprado el Mod`)
            } else {
                logger.info(`${event.userDisplayName} is already a Mod`)
                await TwitchService.cancelRedeemption(event.rewardId, event.id)
                await this.twitchBot.say(`#${config.twitch.channels}`, `@${event.userDisplayName} ya eres Mod tontolava!`)
            }
        })
